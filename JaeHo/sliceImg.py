...
def get_sliceImg(image_origin):
    ...
    return slice1, slice2, body_image[0]

# def get_electrode2(image_origin): #전극부분
#     margin = -8  # 원하는 margin
#     image_copy = image_origin.copy()
#     imgray = cv2.cvtColor(image_copy, cv2.COLOR_BGR2GRAY)
#     ret, thr = cv2.threshold(imgray, 127, 255, cv2.THRESH_BINARY)
...
# def get_body(image_origin): #바디부분
...
#     for contour_body in contours_body:
#         x, y, width, height = cv2.boundingRect(contour_body)  # 좌상단 꼭지점 좌표 , width, height
...
#테스트 용.
img = cv2.imread('C:\/finalProject\JaeHo\/001-1.tif')

cut_image1, cut_image2, cut_image3 = get_sliceImg(img)

cv2.imshow('image1', cut_image1)
cv2.imshow('image2', cut_image2)
cv2.imshow('image3', cut_image3)
cv2.waitKey(0)
